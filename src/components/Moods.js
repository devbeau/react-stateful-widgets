...
export default function Moods() {
  /* STEP 1 */
  let [moodValue, setMoodValue] = useState(initialMood);
  
 

  const makeHappy = () => {
    setMoodValue(moodValue = happyMood);
  };
  const makeSad = () => {
    setMoodValue(moodValue = sadMood);
  };
  const reset = () => {
    setMoodValue(moodValue = initialMood);
  };

  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: moodValue === happyMood ? 'royalBlue' : 'crimson', /* STEP 2 */
  };

  return (
    <div className='widget-moods container'>
      <h2>Moods</h2>
      <div style={style}>{moodValue}</div> {/* STEP 3 */}
      <div>
